...
class PostsRepositoryFirestoreTest : FirestoreTest(POSTS_COLLECTION_PATH) {
  ...
  @Test
  fun canAddPostsToRepository() = runTest {
  ...
  @Test
  fun addPostWithExistingIdThrowsException() = runTest {
  ...
  @Test
  fun addPostWithTheCorrectId() = runTest {
    repository.addPost(post1)
  ...
  @Test
  fun canAddMultiplePostsToRepository() = runTest {
    repository.addPost(post1)
    repository.addPost(post2)
    repository.addPost(post3)
    val posts = repository.getAllPosts()
  ...
  @Test
  fun postIdIsUniqueInTheCollection() = runTest {
  ...
  @Test
  fun getNewPostIdReturnsUniqueIDs() = runTest {
    val numberIDs = 100
  ...
  @Test
  fun canDeleteAPostByID() = runTest {
    repository.addPost(post1)
    repository.addPost(post2)
    repository.addPost(post3)
  ...
  @Test
  fun deleteNonExistentPostThrowsException() = runTest {
  ...
  @Test
  fun canEditTheCorrectPostByID() = runTest {
    repository.addPost(post1)
    repository.addPost(post2)
    repository.addPost(post3)
  ...
  @Test
  fun editNonExistentPostThrowsException() = runTest {
    val exception = runCatching { repository.editPost("nonExistentId", post2) }.exceptionOrNull()
    assertTrue(exception is IllegalArgumentException)
    assertEquals("Post with given Id not found", exception?.message)
  }

  @Test
  fun canRetrieveAPostByID() = runTest {
    repository.addPost(post1)
    repository.addPost(post2)
    repository.addPost(post3)
  ...
  @Test
  fun getPostWithNonExistentIdThrowsException() = runTest {
    repository.addPost(post1)
    repository.addPost(post2)
    repository.addPost(post3)
  ...
  @Test
  fun testGetAllPostsByAuthor() = runTest {
  ...
  @Test
  fun testGetAllPostsWhenNoPostsExist() = runTest {
    val posts = repository.getAllPosts()
    assertTrue(posts.isEmpty())
  }

  @Test
  fun testGetAllPostsByAuthorWhenNoPostsExist() = runTest {
    Firebase.auth.signInAnonymously().await()
    val postsByAuthor = repository.getAllPostsByAuthor()
  ...
  @Test
  fun testGetAllPostsByAuthorWhenNoPostsByAuthorExist() = runTest {
    Firebase.auth.signInAnonymously().await()
    val currentUserId = Firebase.auth.currentUser?.uid ?: throw Exception("No current user found")
  ...
  @Test
  fun canCreatePostWithEmptyDescription() = runTest {
  ...
  @Test
  fun getAllPostsByGivenAuthorWhenNoPostsExist() = runTest {
  ...
  @Test
  fun getAllPostsByGivenAuthorWhenPostsExist() = runTest {
  ...
  @Test
  fun getPostWithNonExistentGivenIdThrowsException() = runTest {
