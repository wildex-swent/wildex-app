...
/** Factor to apply to the score of a user if the query is the beginning of the user's name, surname or username */
private const val WORD_START_FACTOR = 4

/** Factor to apply to the score of a user if the query is the end of the user's name, surname or username */
...
class UserIndex(
   ...
   * lowercase, then strings with uppercase letters can get matched to it (i.e leo matches entirely with Leo),
   * but on the contrary, if a query has some uppercase letters in it, it won't get matched with lower
   * case strings (i.e match between Leo and leo is {eo} and not {leo})
   *
   ...
   * @param patterns a list of patterns which are the words of the search query formatted not to discard
   *  good candidates (i.e. not discard Léonard for query leo)
   * @return a scored match which represents the score of a user relative to a search query, null if the
   *  query doesn't match the user
   */
  ...
  /**
   * Computes the score of a user's string representation (name + surname + username) given
   * a matcher which represents the matching characters from the string to a search pattern
   *
   * @param string a user's string representation (name + surname + username)
