...
/**
 * Generic text search engine that supports fuzzy matching with accent-insensitive search and
 * intelligent scoring based on word boundaries.
 *
 ...
 */
class SearchEngine(private val wordStartFactor: Int = 4, private val wordEndFactor: Int = 2) {
   ...
   */
  fun search(query: String, candidates: List<String>, limit: Int): List<ScoredMatch> {
  ...
  private fun patternFor(subQuery: String): Pattern {
    ...
    val flags =
        if (subQuery.none { it.isUpperCase() }) {
  ...
  /**
   * Computes the total score of a string representation relative to a list of patterns which are
   * all the words of a search query turned into a Pattern
   *
   ...
   * @return a scored match which represents the score of a relative to a search query, null if the
   *   query doesn't match
   */
  ...
  /**
   * Computes the score of a string representation given a matcher which represents the matching
   * characters from the string to a search pattern
   *
   * @param string a string representation
